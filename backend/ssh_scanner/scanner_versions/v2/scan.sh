...
Environment:
  ...
  SCAN_ID                Scan session id used for live progress and report upload.
...
upload_report() {
  ...
  "$PYTHON_BIN" - "$BASE_URL/api/report" "$SCAN_ID" "$report_path" <<'PY'
import sys
import urllib.request

url, scan_id, report_path = sys.argv[1:]
...
if [ "$OFFLINE" -eq 0 ] && [ -n "$SCAN_ID" ]; then
